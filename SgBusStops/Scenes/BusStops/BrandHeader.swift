...
public struct BrandHeader: View {
    ...
    var skyGradient: Gradient {
        Gradient(colors: [
            Color("header/sky_start", bundle: .main),
            Color("header/sky_end", bundle: .main)
        ])
    }
        ...
        TimelineView(.animation(paused: !animated)) { context in
            ...
            Canvas { context, size in
                ...
                context.drawLayer { context in
                    ...
                    context.scaleBy(x: scale, y: scale)
                    context.draw(Image("header/layer/8 Road", bundle: .main), at: .zero)
                }
    ...
    struct AnimationDirector {
        ...
        init(timeInterval: TimeInterval) {
            func layerImage(_ name: String) -> Image {
                Image("header/layer/\(name)", bundle: .main)
            }
            ...
            truckImage = Image("header/truck/Bus \(truckFrame + 1)", bundle: .main)
        }
    }
}
