...
struct BusServicArrivalCell: View {
    ...
    @Environment(\.refreshAction) private var refreshAction
    @State private var isExpanded = false

    var body: some View {
        DisclosureGroup(isExpanded: $isExpanded) {
            if let arrival = serviceArrival.nextBus {
                BusArrivalRow(serviceArrival: serviceArrival, arrival: arrival)
            }
    ...
    private func arrivalColumn(title _: String, arrival: BusArrival, rank: Int, date: Date) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 4) {
                ...
                if let typeText = typeText(for: arrival) {
                    Text(typeText)
    ...
    private func arrivalText(for arrival: BusArrival, now: Date) -> some View {
        if let seconds = arrival.arrivalSeconds(now: now) {
            if seconds <= -60 {
                Text(arrival.arrivalDisplayString()).foregroundStyle(Color.orange)
            } else if seconds <= 0 {
                Text(arrival.arrivalDisplayString()).foregroundStyle(Color.green)
            } else if seconds <= 60 {
                Text("\(seconds)s")
            } else {
            ...
            Text("N.A").foregroundStyle(.secondary)
                .task {
                    refreshAction?(value: serviceArrival.serviceNo)
                }
        }
    }
