...
//
    ...
    public struct SoundEffect: Hashable, Sendable {
        ...
        /// #endif
        /// ```
        @MainActor static var audioSession: AVAudioSession?
    ...
    private protocol SoundEffectPlayer: Sendable {
    ...
    private actor AnySoundEffectPlayer: SoundEffectPlayer {
        @MainActor static var shared: AnySoundEffectPlayer = .init()
    ...
    private actor HapticEngineSoundEffectPlayer: SoundEffectPlayer {
        ...
        func unregister(_ audio: SoundEffect) async throws {
            ...
            for url in audio.urls {
                ...
                if registeredSounds[url]?.count ?? 0 <= 0 {
                    #if DEBUG
    ...
    private actor AVSoundEffectPlayer: SoundEffectPlayer {
        ...
        private func setUp() async throws {
            if didSetUp { return }
            defer { didSetUp = true }
            let session = await MainActor.run { SoundEffect.audioSession }
        ...
        func register(_ audio: SoundEffect) async {
            try? await setUp()
