...
func clamp<F: FloatingPoint>(_ value: F) -> F {
    clamp(0, value, 1)
}

func map<T: FloatingPoint>(value: T, inMin: T, inMax: T, outMin: T, outMax: T) -> T {
    (value - inMin) * (outMax - outMin) / (inMax - inMin) + outMin
}

func lerp<T: FloatingPoint>(_ value: T, outMin: T, outMax: T) -> T {
    map(value: value, inMin: 0, inMax: 1, outMin: outMin, outMax: outMax)
}
...
func easeInCubic(_ t: CGFloat) -> CGFloat {
    t * t * t
}
...
func easeInOutQuart(_ t: CGFloat) -> CGFloat {
    if t < 0.5 {
        8 * pow(t, 4)
    } else {
        -1 / 2 * pow(2 * t - 2, 4) + 1
...
func cubicBezier(x1: CGFloat, y1: CGFloat, x2: CGFloat, y2: CGFloat) -> (CGFloat) -> CGFloat {
    ...
    func binarySubdivide(_ x: CGFloat, _ x1: CGFloat, _ x2: CGFloat) -> CGFloat {
        ...
        while true {
            ...
            if abs(currentX) > epsilon, i < maxIterations {
