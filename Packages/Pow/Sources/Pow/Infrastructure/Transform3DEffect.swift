...
struct Transform3DEffect: GeometryEffect, Animatable {
    var animatableData: AnimatablePair<TRS, AnimatablePair<Anchor3D, Double>> = .zero

    init(
        translation: simd_double3 = .zero,
        rotation: simd_quatd = simd_quatd(angle: 0, axis: .zero),
        scale: simd_double3 = [1, 1, 1],
        anchor: UnitPoint = .center,
        anchorZ: Double = 0,
        perspective: Double = 1
    ) {
        animatableData.first = TRS(translation: translation, rotation: rotation, scale: scale)
        animatableData.second.first = Anchor3D(xy: anchor, z: anchorZ)
        animatableData.second.second = perspective
    }
...
struct ShadedTransform3DEffect: ViewModifier, Animatable {
    ...
    func body(content: Content) -> some View {
        ...
        let n: CGFloat = if dot(normal, screenVector) >= 0 {
            dot(lightVector, normal)
        } else {
            dot(lightVector, -normal)
        }
    ...
    struct Transform3DEffect_Preview: PreviewProvider {
        struct Preview: View {
            @State
            private var anchor: (x: Double, y: Double, z: Double) = (0.5, 0.5, 0)

            @State
            private var translation: (x: Double, y: Double, z: Double) = (0, 0, 0)

            @State
            private var angle: (x: Angle, y: Angle, z: Angle) = (.zero, .zero, .zero)

            @State
            private var scale: (x: Double, y: Double, z: Double) = (1, 1, 1)

            @State
            private var perspective: CGFloat = 0.16
