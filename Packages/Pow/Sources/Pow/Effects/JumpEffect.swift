...
struct JumpSimulationModifier: ViewModifier, Simulative {
    var impulseCount: Int

    var initialVelocity: CGFloat = 0

    private let spring: Spring = .init(zeta: 1 / 3, stiffness: 100 * 1)
    ...
    init(height: Double, impulseCount: Int) {
        ...
        initialVelocity = -(height / peakHeight)
        targetHeight = height
    }
