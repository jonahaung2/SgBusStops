...
public extension AnyChangeEffect {
    ...
    enum SpinRate {
        ...
        fileprivate var initialVelocity: Angle {
            switch self {
            case .fast: .degrees(900)
            case .default: .degrees(360)
            case let .velocity(initial, _, _): initial
            }
        }

        fileprivate var maximumVelocity: Angle {
            switch self {
            case .fast: .degrees(360 * 4)
            case .default: .degrees(360 * 2)
            case let .velocity(_, maximum, _): maximum
            }
        }
        ...
        fileprivate var additionalVelocity: Angle {
            ...
            case .fast: .degrees(900)
            case .default: .degrees(360)
            case let .velocity(_, _, additional): additional
            }
        }
    }
    ...
    static func spin(
        ...
        multiplier speedBoost: CGFloat = 0.0,
...
struct SpinSimulationModifier: ViewModifier, Simulative {
    var impulseCount: Int

    var initialVelocity: CGFloat = 0

    let spring: Spring = .init(zeta: 1 / 2, stiffness: 7)
    ...
    private func update(_ step: Double) {
        ...
        if abs(angleVelocity.degrees) > 240 {
            ...
            newVelocity = angleVelocity.degrees * (0.99 - additionalSpeed)
